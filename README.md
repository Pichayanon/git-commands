...
1. When using Git locally, what are these?  Define each one in a sentence
   * Staging area - The place to record file before commit.
   * Working copy - The copy of files you work on.
   * master - The name of the default branch.
   * HEAD - The label of latest commit in current branch.

2. When you install git on a new machine (or in a new user account) you should perform these 2 git commands to tell git your name and email.  These values are used in commits that you make:
   ```
   # Git configuration commands for a new account
   git config --global user.name "Pichayanon Toojinda"
   git config --global user.email kewkunggt@gmail.com
   ```
...
1. Create `dev1` branch and switch to this branch.
   ```
   git checkout -b dev1
   ```
2. Delete `dev1` branch.
   ```
   git branch -d dev1
   ```
3. Now that you are in the `master` branch , you want to merge `dev2` branch into the `master` branch.
   ```
   git merge dev2
   ```
4. You want to show the names of the files that are different in `dev1` branch and `dev2` branch.
   ```
   git diff dev1 dev2 --name-only
   ```


## Favorites

1. Display the state of your repository. It shows changed files.
   ```
   git status
   ```
2. You added the wrong file to the staging area, and you need to remove it from the staging area.
   ```
   git reset filename
   ```
...
* [ATLASSIAN Bitbucket](https://www.atlassian.com/git/tutorials/learn-git-with-bitbucket-cloud) Explains how to use git and has pictures to make it easy to understand.
* [W3Schools](https://www.w3schools.com/git/default.asp?remote=github): Explains git commands and has examples to study and try.
